...
/**
 * Get account balance - FIXED FOR BIGINT
 */
const getBalance = async (address) => {
  try {
    const accountInfo = await algodClient.accountInformation(address).do();
    return {
      balance: Number(accountInfo.amount) / 1000000, // Convert microAlgos to Algos (BigInt fix)
...
/**
 * Reward user with platform tokens - FIXED FOR BIGINT
 */
const rewardTokens = async (recipientAddress, amount, reason) => {
  try {
    ...
    const txn = algosdk.makePaymentTxnWithSuggestedParamsFromObject({
      from: treasuryAccount.addr,
      to: recipientAddress,
      amount: Math.round(amount * 1000000), // Convert to microAlgos (BigInt fix)
      note: algosdk.encodeObj({
        type: 'reward',
        reason: reason,
        date: new Date().toISOString()
